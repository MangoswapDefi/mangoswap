import { CurrencyAmount, JSBI, Token, Trade } from '@mangoswap-libs/sdk'
    ...
    <Container>
      ...
      <AppBody>
        <Wrapper id="swap-page">
          ...
          />
          <PageHeader
            title={intl('swap.exchange','Exchange')}
            description={intl('swap.exchangeInfo', 'Trade tokens in an instant')}
          />
          <Zoom zoom={0.9}>
          <CardBody>
            <AutoColumn gap="md">
              <CurrencyInputPanel
                label={
                  independentField === Field.OUTPUT && !showWrap && trade
                    ? intl('swap.FromEstimated', 'From (estimated)')
                    : intl('swap.from', 'From')
                }
                ...
                <AutoRow justify={isExpertMode ? 'space-between' : 'center'} style={{ padding: '0 1rem' }}>
                  <ArrowWrapper clickable>
                    <IconButton
                      ...
                      scale="sm"
                    >
                      <ArrowDownIcon color="#BDBDBD" width="24px" />
                    </IconButton>
                  </ArrowWrapper>
                  {recipient === null && !showWrap && isExpertMode ? (
                    <LinkStyledButton id="add-recipient-button" onClick={() => onChangeRecipient('')}>
                      + {intl('swap.addSend', 'Add a send (optional)')}
                    </LinkStyledButton>
                  ) : null}
                </AutoRow>
              </AutoColumn>
              <CurrencyInputPanel
                ...
                label={
                  independentField === Field.INPUT && !showWrap && trade
                    ? intl('swap.toEstimated', 'To (estimated)')
                    : intl('swap.to', 'To')
                }
                showMaxButton={false}
                  ...
                  <AutoColumn gap="4px">
                    {Boolean(trade) && (
                      <RowBetween align="center">
                        <Text fontSize="14px">{intl('swap.price', 'Price')}</Text>
                        <TradePrice
                    ...
                    {allowedSlippage !== INITIAL_ALLOWED_SLIPPAGE && (
                      <RowBetween align="center">
                        <Text fontSize="14px">{intl('swap.slippageTolerance', 'Slippage Tolerance')}</Text>
            ...
            <BottomGrouping>
              {!account ? (
                ...
                <GreyCard style={{ textAlign: 'center' }}>
                  <Text mb="4px">{intl('swap.insufficientLiquidity', 'Insufficient liquidity for this trade.')}</Text>
                </GreyCard>
              ) : showApproveFlow ? (
                <RowBetween>
                  <Button
                  ...
                  >
                    {approval === ApprovalState.PENDING ? (
                      <AutoRow gap="6px" justify="center">
                        {intl('swap.approving', 'Approving')} <Loader stroke="white" />
                      </AutoRow>
                    ) : approvalSubmitted && approval === ApprovalState.APPROVED ? (
                      intl('swap.approved', 'Approved')
                    ) : (
                  ...
                  >
                    {priceImpactSeverity > 3 && !isExpertMode
                      ? intl('swap.priceImpactHigh', 'Price Impact High')
                      : priceImpactSeverity > 2 ? intl('swap.swapAnyway') : intl('swap.swap')
                    }
                  </Button>
                </RowBetween>
              ) : (
