...
contract IFO is OwnableUpgradeSafe {
    ...
    event Swapped(uint indexed index, address indexed sender, uint amount);
    event Claimed(uint indexed index, address indexed sender, uint amount0, uint amount1);
    event UserClaimed(uint indexed index, address indexed sender, uint amount0, uint amount1);

    function initialize(address router_, address factory_) public initializer {
    ...
    function create(CreateReq memory poolReq) external onlyOwner {
        ...
        _token0.safeTransferFrom(poolReq.seller, address(this), poolReq.amountTotal0);
        require(
    ...
    function swap(uint index, uint amount1) external
        isPoolExist(index)
        ...
        lock
    {
        ...
        emit Swapped(index, sender, amount1);
    }
        ...
        lock
    {
        ...
        require(!teamClaimed[pool.seller][index], "claimed");
        teamClaimed[pool.seller][index] = true;
        ...
        if (amountSwap0P[index] < pool.amountTotal0) {
            ...
            IERC20(pool.token0).safeTransfer(pool.seller, unSwapAmount0);
        }
        ...
        if (swapAmount1 > 0) {
            IERC20(pool.token1).safeApprove(router, swapAmount1);
            IUniswapV2Router02(router).removeLiquidity(
                IUniswapV2Pair(pool.token1).token0(),
                IUniswapV2Pair(pool.token1).token1(),
                swapAmount1, // liquidity
                0, // amountAMin
                0, // amountBMin
                pool.seller, // to
                now.add(20 minutes) // deadline
            );
        }

        emit Claimed(index, msg.sender, unSwapAmount0, swapAmount1);
    }
