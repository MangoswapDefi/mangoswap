    ...
    const bulkResults = await Promise.all(
      current &&
        ...
        current?.data?.tokens.map(async (token) => {
          ...
          if (!oneDayHistory) {
            const oneDayResultData = await client.query({
              query: TOKEN_DATA(token.id, oneDayBlock),
              fetchPolicy: 'cache-first',
            })
            oneDayHistory = oneDayResultData.data.tokens[0]
          }
          if (!twoDayHistory) {
            const twoDayResultData = await client.query({
              query: TOKEN_DATA(token.id, twoDayBlock),
              fetchPolicy: 'cache-first',
            })
            twoDayHistory = twoDayResultData.data.tokens[0]
          }
...
const getTokenData = async (address, ethPrice, ethPriceOld) => {
  ...
  try {
    ...
    if (!oneDayData) {
      ...
      oneDayData = oneDayResultData.data.tokens[0]
    }
    if (!twoDayData) {
      const twoDayResultData = await client.query({
        query: TOKEN_DATA(address, twoDayBlock),
      ...
      twoDayData = twoDayResultData.data.tokens[0]
    }
    ...
    if (data.id === '0x7fc66500c84a76ad7e9c93437bfc5ac33e2ddae9') {
      ...
      const resultIndex = aaveData.data.pairs[0]
      data.totalLiquidityUSD = parseFloat(resultIndex.reserveUSD) / 2
      data.liquidityChangeUSD = 0
      data.priceChangeUSD = 0
    }
  } catch (e) {
...
const getIntervalTokenData = async (tokenAddress, startTime, interval = 3600, latestBlock) => {
  ...
  try {
    ...
    let index = 0
    const resultDataKeys = Object.keys(result);
    for (let i = 0; i < resultDataKeys.length; ++i) {
      const brow = resultDataKeys[i];
...
export function Updater() {
  ...
  useEffect(() => {
    ...
    async function getData () {
      ...
      const topTokens = await getTopTokens(ethPrice, ethPriceOld)
      // console.log('---topTokens---', topTokens)
      if (topTokens)updateTopTokens(topTokens)
    }
    // console.log('--ethPrice, ethPriceOld--', ethPrice, ethPriceOld)
    if (isNotVoid(ethPrice) && isNotVoid(ethPriceOld)) getData()
