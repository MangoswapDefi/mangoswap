...
function PairList({ pairs, color, disbaleLinks, maxItems = 10 }) {
  ...
  const ListItem = ({ pairAddress, index }) => {
    ...
    if (pairData && pairData.token0 && pairData.token1) {
      ...
      return (
        <DashGrid style={{ height: '48px' }} disbaleLinks={disbaleLinks} focus>
          <DataText area="name" fontWeight="500">
            ...
            <DoubleTokenLogo
              ...
              margin={!below740}
            />
            <CustomLink style={{ marginLeft: '20px', whiteSpace: 'nowrap' }} target="_blank" href={`${LINK_INFO_BASE_URL}/pair/${pairAddress}`} color={color}>
              <FormattedName
          ...
          {!below600 && <Space size="middle">
            <ActionIconLink to={`/add/${pairData.token0.id}/${pairData.token1.id}`}>
    ...
    Object.keys(pairs)
      ...
      .map((pairAddress, index) => {
        return (
          pairAddress && (
            <div key={pairAddress}>
              <ListItem key={pairAddress} index={(page - 1) * ITEMS_PER_PAGE + index + 1} pairAddress={pairAddress} />
              {/* <Divider /> */}
            </div>
          )
        )
    ...
    <ListWrapper>
      ...
      >
        <Flex alignItems="center" justifyContent="flexStart">
          <TYPE.Main area="name">{intl('home.name')}</TYPE.Main>
        </Flex>
        <Flex alignItems="center" justifyContent="flexEnd">
          ...
          >
            {intl('home.liquidity')} {sortedColumn === SORT_FIELD.LIQ ? (!sortDirection ? '↑' : '↓') : ''}
          </ClickableText>
        </Flex>
        ...
        {!below1080 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            ...
            >
              {intl('home.Volume7d')} {sortedColumn === SORT_FIELD.VOL_7DAYS ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
        {!below1080 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            ...
            >
              {intl('home.fees24h')} {sortedColumn === SORT_FIELD.FEES ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
        {!below1080 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            ...
            >
              {intl('home.fees1y')} {sortedColumn === SORT_FIELD.APY ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
            <QuestionHelper text={intl('home.volumeAnnualized')} />
          </Flex>
        )}
        {!below600 && <Flex alignItems="center">
          <ClickableText
            area="action"
          >
